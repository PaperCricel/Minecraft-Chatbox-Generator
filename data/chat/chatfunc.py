...
def insert_en():
    ...
    if IsShortSentenceRapid == True:
        cbIsShortSentenceRapid.select()
    cbGenerateSkip.deselect()
    if GenerateSkip == True:
        cbGenerateSkip.select()
...
def import_data():
    global f
    f = tkfd.askopenfilename(
        initialdir=here + "/functions/",
        title="選擇backup檔案",
...
def generate_cmd():
    ...
    for i in range(len(dir_all) - 1):
        ...
        try:
            if not os.path.isdir(Current_file + dir_this):
                os.mkdir(Current_file + dir_this)
            if i == len(dir_all) - 2:
                # 生成skip資料夾
                if GenerateSkip == True and not os.path.isdir(
                    Current_file + dir_this + "skip/"
                ):
                    os.mkdir(Current_file + dir_this + "skip/")
                # 生成basic資料夾
                if not os.path.isdir(Current_file + dir_this + "basic/"):
                    os.mkdir(Current_file + dir_this + "basic/")
        except:
    ...
    if error == False:
        error = er_file.error_2(win, FilePlace, lbFilePlace, enFilePlace, error)
    if error == False:
        error = er_file.error_3(win, Ta, lbTa, lbTa1, enTa, error)
    if error == False:
        error = er_file.error_4(win, Text, lbText, enText, error)
    if error == False:
        error = er_file.error_5(win, Prefix, lbPrefix, enPrefix, error)
    if error == False:
        error = er_file.error_6(win, Serfix, lbSerfix, enSerfix, error)
...
ttk.Separator(win, orient=tk.HORIZONTAL).grid(
    row=1, column=0, columnspan=2, sticky="new", pady=10
)

# Basic Options
BasicOptions = ttk.Frame(win)
ttk.Separator(BasicOptions, orient=tk.VERTICAL).grid(
    row=0, column=1, rowspan=100, sticky="nsw", padx=20
)
...
lbSpace = tk.Label(BasicOptions, text="短空格", font=("Microsoft JhengHei", 12)).grid(
    row=3, column=0, pady=3
)
lbSpaceLong = tk.Label(BasicOptions, text="長空格", font=("Microsoft JhengHei", 12)).grid(
...
lbPos2 = tk.Label(
    FmPos, text=" run summon chat box here", font=("Microsoft JhengHei", 12)
)
...
enVisiblePlayer = tk.Entry(
    BasicOptions, font=("Microsoft JhengHei", 12), width=20
...
tk.Label(FmIsDotSlow, text="每個...會變成", font=("Microsoft JhengHei", 12)).grid(
    row=1, column=0
)
varIsDotSlow = tk.BooleanVar()
cbIsDotSlow = tk.Checkbutton(
    FmIsDotSlow,
    text="...需不需要變速",
    variable=varIsDotSlow,
...
tk.Label(FmIsDotSlow, text=" ticks 間隔", font=("Microsoft JhengHei", 12)).grid(
    row=1, column=2
)
FmIsDotSlow.grid(row=0, column=0, sticky="w", padx=10, pady=5)

FmIsShortSentenceRapid = tk.Frame(MoreOptions, bd=1, relief="solid")
tk.Label(FmIsShortSentenceRapid, text="句子只有", font=("Microsoft JhengHei", 12)).grid(
    row=1, column=0
)
enIsShortSentenceRapid1 = tk.Entry(
    FmIsShortSentenceRapid, width=2, font=("Microsoft JhengHei", 12)
)
enIsShortSentenceRapid1.grid(row=1, column=1)
tk.Label(
    FmIsShortSentenceRapid, text="個字的時候，會變成", font=("Microsoft JhengHei", 12)
).grid(row=1, column=2)
enIsShortSentenceRapid2 = tk.Entry(
    FmIsShortSentenceRapid, width=2, font=("Microsoft JhengHei", 12)
)
enIsShortSentenceRapid2.grid(row=1, column=3)
tk.Label(FmIsShortSentenceRapid, text="ticks 間隔", font=("Microsoft JhengHei", 12)).grid(
    row=1, column=4
)
varIsShortSentenceRapid = tk.BooleanVar()
cbIsShortSentenceRapid = tk.Checkbutton(
    FmIsShortSentenceRapid,
    text="太少字是否要加速",
    variable=varIsShortSentenceRapid,
    font=("Microsoft JhengHei", 12),
)
...
tk.Label(FmGenerateSkip, text="ticks 的冷卻時間", font=("Microsoft JhengHei", 12)).grid(
    row=1, column=2
)
varGenerateSkip = tk.BooleanVar()
cbGenerateSkip = tk.Checkbutton(
    FmGenerateSkip,
    text="是否要生成Skip文件",
    variable=varGenerateSkip,
    font=("Microsoft JhengHei", 12),
)
...
ttk.Separator(FmTextDisplay, orient=tk.VERTICAL).grid(
    row=0, column=1, rowspan=100, sticky="nsw", padx=20
)
tk.Label(FmTextDisplay, text="顯示形式", font=("Microsoft JhengHei", 12)).grid(
    row=0, column=0, pady=3
)
tk.Label(FmTextDisplay, text="文字靠邊", font=("Microsoft JhengHei", 12)).grid(
    row=1, column=0, pady=3
)
tk.Label(FmTextDisplay, text="文字大小", font=("Microsoft JhengHei", 12)).grid(
    row=4, column=0, pady=3
)
tk.Label(FmTextDisplay, text="最長句子", font=("Microsoft JhengHei", 12)).grid(
    row=5, column=0, pady=3
)
tk.Label(FmTextDisplay, text="文字透明度", font=("Microsoft JhengHei", 12)).grid(
    row=6, column=0, pady=3
)

bb = tk.StringVar()
rbb = ["center(預設)", "fixed", "horizontal"]
enBillboard = ttk.Combobox(
    FmTextDisplay,
    values=rbb,
    textvariable=bb,
    state="readonly",
...
cbBackground = tk.Checkbutton(
    FmTextDisplay,
    text="是否有背景框",
    variable=varBackground,
    font=("Microsoft JhengHei", 12),
)
...
cbSeeThrough = tk.Checkbutton(
    FmTextDisplay,
    text="文字顯示穿透過方塊",
    variable=varSeeThrough,
    font=("Microsoft JhengHei", 12),
)
...
ttk.Separator(MoreOptions, orient=tk.HORIZONTAL).grid(
    row=4, column=0, columnspan=1000, sticky="new", pady=20
)
...
tk.Label(
    FmGenerateActive, text="快速微調生成\n(建議搞懂後再使用.w.)", font=("Microsoft JhengHei", 12)
).grid(row=0, column=0, columnspan=1000, sticky="new", pady=(0, 10))
Generates = ["Start", "End", "Setup", "Tick", "Space", "Backup"]
for i in range(len(Generates)):
    btGenerateQuick = tk.Button(
        FmGenerateActive,
        text=f"生成{Generates[i]}",
        command=lambda m=i: generate_cmd_quick(m),
        font=("Microsoft JhengHei", 10),
        relief="solid",
    )
...
# ----- End ----- #
ttk.Separator(win, orient=tk.HORIZONTAL).grid(
    row=17, column=0, columnspan=2, sticky="new", pady=20
)
...
BtBasicOption = tk.Button(
    win, text="基礎設定", command=PageBasicOptions, font=("Microsoft JhengHei", 12)
).grid(row=0, column=0, sticky="we")
BtMoreOption = tk.Button(
    win, text="更多設定", command=PageMoreOptions, font=("Microsoft JhengHei", 12)
...
# file
file_menu = aw.File(
    top_menu, "檔案", ["導入Backup檔案", "儲存Backup檔案"], [import_data, save_data]
)
...
Mode_menu = aw.Mode(
    top_menu,
    "模式",
    ["1.19.4前漂浮文字", "1.19.4後漂浮文字", "Actionbar文字"],
    [area, text, marker],
    Mode,
    mode_temp,
)
