...
def start(
    ...
    with open(File_start, "w+", encoding="utf-8") as f:
        ...
        if Mode == 0:
            f.write(
                f'execute {Pos} run summon area_effect_cloud ~ ~ ~ {{Tags:[chat,master,{Ta}],CustomNameVisible:1b,CustomName:\'{{"text":""}}\',Duration:1000000000}}\n'
            )
        elif Mode == 1:
            f.write(
                f'execute {Pos} run summon text_display ~ ~ ~ {{Tags:[chat,master,{Ta}],CustomNameVisible:0b,text:\'{{"text":""}}\''
            )
            if Billboard == 0:
                f.write(f',billboard:"center"')
            elif Billboard == 1:
                f.write(f',billboard:"fixed"')
            elif Billboard == 2:
                f.write(f',billboard:"horizontal"')

            if Alignment == 0:
                f.write(f',alignment:"center"')
            elif Alignment == 1:
                f.write(f',alignment:"left"')
            elif Alignment == 2:
                f.write(f',alignment:"right"')

            if SeeThrough == True:
                f.write(f",see_through:1b")

            if Background == False:
                f.write(f",background:0")
            if LineWidth != 200:
                f.write(f",line_width:{LineWidth}")
            ...
            f.write(
                f'summon marker 0 0 0 {{Tags:[chat,master,{Ta}],CustomNameVisible:0b,CustomName:\'{{"text":""}}\'}}\n'
            )
        f.write(f"\n")
        f.write(f"# 文字設定\n")
        f.write(f"function chat:{FilePlace}basic/setup\n")
        if Mode == 1 and (float(Scale1) != 1.0 or float(Scale2) != 1.0):
            f.write(f"schedule function chat:{FilePlace}basic/transformation 1t\n")
        f.write(f"\n")
        f.write(f"# 聊天循環\n")
    ...
    if Mode == 1 and (float(Scale1) != 1.0 or float(Scale2) != 1.0):
        with open(File_Trans, "w+", encoding="utf-8") as f:
            f.write(
                f"execute as @e[type=#minecraft:text,tag={Ta}] run data merge entity @s {{transformation:{{scale:[{Scale1}f,{Scale2}f,1.0f]}}}}"
            )
    else:
        try:
    ...
    with open(File_setup, "w+", encoding="utf-8") as f:
        ...
        for i in Text:
            ...
            for t in range(int(len(i))):
                ...
                for r in range(len(Textcolor_color)):
                    Texts[-1] = Texts[-1].replace(
                        "#b", '"},{"color":"' + Textcolor_color[r] + '","text":"', 1
                    )
        ...
        for i in range(int(len(Texts))):
            if Texts[i] != "$command$:long_string":
                f.write(
                    f'data modify storage {Ta} Text append value \'["",{Prefix}{{"text":"{Texts[i]}"}}{Serfix}]\''
                )
                f.write("\n")
...
def space(
    ...
    with open(File_space, "w+", encoding="utf-8") as f:
        ...
        if GenerateSkip == True:
            f.write(
                f"execute store result score #len({Ta}_space) text_space run data get storage {Ta} Skip[0]\n"
            )
        f.write(f"data modify block {Sign_Pos} Text1 set from storage {Ta} Text[0]\n")
        f.write(f"\n")
        f.write(f"# ----- 取代文字/間隔 ----- #\n")
        f.write(
            f"execute as @e[type=#minecraft:text,tag={Ta}] run data modify entity @s "
        )
        if Mode == 1:
            f.write("text")
        else:
        ...
        f.write(
            f"execute as @e[type=#minecraft:text,tag={Ta}] store result score @s text_space run data get storage {Ta} Space[0]\n"
        )
        if Mode == 2:
            f.write(
                f'title {VisiblePlayer} actionbar [{{"selector":"@e[type=#minecraft:text,tag={Ta}]"}}]\n'
            )
        f.write(f"\n")
        f.write(f"# ----- 刪除文字/間隔 ----- #\n")
        ...
        if GenerateSkip == True:
            f.write(
                f"execute if score #len({Ta}) text_space = #len({Ta}_space) text_space run data remove storage {Ta} Skip[0]\n"
            )
            f.write(
                f"execute if score #len({Ta}) text_space = #len({Ta}_space) text_space run scoreboard players set @e[type=#minecraft:text,tag={Ta}] skip_cd 0\n"
            )
        f.write("\n")
        f.write("# ----- 結束 ----- #\n")
        f.write(
            f"execute unless data storage {Ta} Text[0] run function chat:{FilePlace}-end\n"
        )
        f.write("\n")
        f.write("# ----- 音效 ----- #\n")
        if Sound != "":
            if Mode != 2:
                f.write(
                    f"execute as @e[type=#minecraft:text,tag={Ta}] at @s if data storage {Ta} Text[0] run {Sound}\n"
                )
            else:
                f.write(
                    f"execute as {VisiblePlayer} at @s if data storage {Ta} Text[0] run {Sound}\n"
                )
        f.write("\n")
        f.write("# ----- 指令擴充 ----- #\n")
        if len(Events) > 0:
            for i in Events:
                f.write(
                    f"execute if score #len({Ta}) text_space matches {i} run say [cmd triggered.]\n"
                )
        f.close
...
def end(File_end, Ta, FilePlace):
    with open(File_end, "w+", encoding="utf-8") as f:
        ...
        f.write("# 刪除\n")
        f.write(
            f"execute as @e[type=#minecraft:text,tag={Ta}] run function chat:void\n"
        )
        f.write("\n")
        f.write("# ----- 指令擴充 ----- #\n")
        f.close
...
def skip(File_skip, File_skipl, File_skips, Ta, SkipCoolDown, FilePlace):
    with open(File_skip, "w+", encoding="utf-8") as f:
        ...
        f.write("# ----- 間距計算 ----- #\n")
        f.write(
            f"execute store result score #skip text_space run data get storage {Ta} Skip[0]\n"
        )
        f.write(
            f"scoreboard players operation #skip text_space -= #len({Ta}) text_space\n"
        )
        f.write(f"scoreboard players remove #skip text_space 1\n")
        f.write(
            f"scoreboard players operation #len({Ta}) text_space += #skip text_space\n"
        )
        f.write("\n")
        f.write("# ----- 刪除文字 ----- #\n")
        f.write(f"data remove storage {Ta} Skip[0]\n")
        f.write(
            f"execute if score @e[type=#minecraft:text,tag={Ta},tag=master,limit=1] skip_cd matches {SkipCoolDown}.. if score #skip text_space matches 1.. run function chat:{FilePlace}skip/loop\n"
        )
        f.write("\n")
        f.write("# ----- 下一段開始 ----- #\n")
        f.write(
            f"execute as @e[type=#minecraft:text,tag={Ta},tag=master,limit=1] if score @s skip_cd matches {SkipCoolDown}.. run function chat:{FilePlace}basic/space_loop\n"
        )
        f.write(
            f"scoreboard players set @e[type=#minecraft:text,tag={Ta},scores={{skip_cd={str(SkipCoolDown)}..}}] skip_cd 0\n"
        )
        f.close
    with open(File_skipl, "w+", encoding="utf-8") as f:
        ...
        f.write("\n")
        f.write(
            f"execute if score #skip text_space matches 1.. run function chat:{FilePlace}skip/loop\n"
        )
        f.close
    with open(File_skips, "w+", encoding="utf-8") as f:
        ...
        f.write("# ----- 存入Skip分數 ----- #\n")
        f.write(
            f"execute store result score Skip text_space run data get storage {Ta} Space\n"
        )
        f.write("scoreboard players add Skip text_space 1\n")
        f.write(
            f"execute store result storage {Ta} tempskip int 1 run scoreboard players get Skip text_space\n"
        )
...
def tick(File_tick, Ta, FilePlace):
    with open(File_tick, "r", encoding="utf-8") as f:
        temp = []
        if (
            not f"execute if entity @s[tag={Ta}] run function chat:{FilePlace}basic/space_loop"
            in f.read()
        ):
    ...
    with open(File_tick, "r", encoding="utf-8") as f:
        for i in f.readlines():
            if i == "\n" or i[0] == "#":
                ...
                __file__.replace("\\", "/").replace("chat/ctgnrt/main.py", "")
                + i[i.find("function ") + 9 :]
                .replace(":", "/functions/")
                .replace("\n", "")
                + ".mcfunction"
